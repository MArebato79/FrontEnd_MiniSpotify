...
export const LibraryPage = () => {
    ...
    const [myArtistData, setMyArtistData] = useState(null); // Para guardar álbumes y canciones si es artista

    useEffect(() => {
        // 1. Cargar Playlists (Todos los usuarios)
        getMyPlaylists().then(data => setPlaylists(data || []));

        // 2. Si es Artista, cargar sus datos (álbumes y canciones)
        if (user?.artistId) {
    ...
    return (
        <div className="p-8 text-white pb-24">
            ...
            {/* SECCIÓN 1: MIS PLAYLISTS (PARA TODOS) */}
            ...
            {/* SECCIONES DE ARTISTA (SOLO SI ES ARTISTA) */}
            {user?.artistId && myArtistData && (
                <>
                    {/* SECCIÓN 2: MIS ÁLBUMES */}
                    <section className="mb-12">
                        <h2 className="text-2xl font-bold mb-4 flex items-center gap-2 border-b border-white/10 pb-2">
                    ...
                    {/* SECCIÓN 3: MIS CANCIONES (RESUMEN) */}
                    <section>
                        <h2 className="text-2xl font-bold mb-4 flex items-center gap-2 border-b border-white/10 pb-2">
                            <Music className="text-purple-500" /> Mis Canciones
                        </h2>
                        <div className="bg-white/5 rounded-lg p-4">
                            {/* Como la API de getArtistById suele traer álbumes anidados, aplanamos la lista para ver todas las canciones */}
