...
export const AlbumPage = () => {
  ...
  // Función para ir al perfil del artista sin reproducir la canción
  const goToArtist = (e, artistId) => {
    e.stopPropagation(); // Detiene el evento para que no suene la música
    if (artistId) {
      ...
      <div className="flex flex-col md:flex-row items-end gap-6 p-8 bg-gradient-to-b from-transparent to-black/20">
        ...
        <div className="flex flex-col gap-2 w-full">
          ...
          <div className="flex items-center flex-wrap gap-2 text-sm font-medium text-gray-300 mt-2">
            {album.artista?.imagenUrl ? (
              <img src={album.artista.imagenUrl} className="w-6 h-6 rounded-full object-cover" alt="Avatar Artista" />
            ) : (
              <div className="w-6 h-6 rounded-full bg-gray-500 flex items-center justify-center"><User size={14} /></div>
            )}

            {/* Nombre del Artista en la cabecera (Clicable) */}
            <span
              className="text-white font-bold hover:underline cursor-pointer"
              onClick={(e) => goToArtist(e, album.artista?.id)}
            >
              ...
              >
                ...
                <div className="flex items-center gap-4 overflow-hidden">
                  ...
                  <div className="flex flex-col justify-center min-w-0">
                    ...
                    <div
                      className="text-gray-400 text-sm truncate group-hover:text-white transition-colors"
                    ...
                    >
                      ...
                      <span
                        ...
                        onClick={(e) => goToArtist(e, album.artista?.id)}
                      >
                        {track.artistaNombre || album.artista?.nombre || "Desconocido"}
                      </span>

                      {/* Colaboradores (Feats) */}
                      {track.colaboradores && track.colaboradores.length > 0 && (
                        <>
                          <span className="text-gray-500"> ft. </span>
                          {track.colaboradores.map((feat, index, array) => (
                            <span key={feat.id || index}>
                              <span
                                className="hover:text-white hover:underline cursor-pointer"
                                onClick={(e) => goToArtist(e, feat.id)}
                              >
                                {feat.nombre}
                              </span>
