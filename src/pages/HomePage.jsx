...
export const HomePage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [artistsData, albumsData, playlistsData, songsResponse, followedData] = await Promise.all([
          getAllArtists(),
          getAllAlbums(),
          getAllPlaylists(),
          api.get("/canciones"),
  ...
  return (
    <div className="flex flex-col h-full bg-gradient-to-b from-[#1e1e1e] to-[#121212]">
      ...
      <div className="px-8 pb-32 space-y-12 overflow-y-auto custom-scrollbar mt-4">
            ...
            {canciones.slice(0, 8).map((song) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-center">
                  ...
                  <p className="text-xs text-gray-400 truncate">{song.artista?.nombre || "Desconocido"}</p>
                </div>
            ...
            {albums.slice(0, 5).map((album) => (
              ...
              >
                <div className="relative mb-4">
                    <img src={album.imagenUrl || "https://placehold.co/200"} className="w-full aspect-square object-cover rounded-md shadow-lg" />
        ...
        {/* --- SECCIÓN AÑADIDA: TODOS LOS ARTISTAS DEL CATÁLOGO --- */}
        <section>
          <h2 className="text-2xl font-bold text-white mb-6 flex items-center gap-2">
